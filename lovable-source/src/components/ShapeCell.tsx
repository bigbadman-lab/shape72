...
import { padShapeId, type Shape } from "@/lib/shapes";
...
export function ShapeCell({ shape, walletConnected, onOpen }: ShapeCellProps) {
    ...
    >
      ...
      <div className="absolute inset-0 grid place-items-center px-8 pb-10 pt-6 sm:px-10">
        {broken ? (
          ...
          <span className="relative flex max-h-[72%] w-3/5 transition-transform duration-300 ease-out group-hover:scale-[1.06]">
            ...
            />
            <span aria-hidden className="shape-mask" style={{ ["--shape-src" as string]: `url("${shape.image}")` }} />
          </span>
        )}
      </div>

      {/* Understated metadata strip. */}
      <div className="absolute inset-x-0 bottom-0 flex items-center justify-between px-3 pb-4 font-display text-[10px] uppercase tracking-[0.18em] sm:px-5">
      ...
      {rewardsActive && (
        <div className="absolute right-3 top-4 flex items-center gap-2 font-display text-[9px] uppercase tracking-[0.16em] text-primary sm:right-5 sm:text-[10px] sm:tracking-[0.18em]">
          <span aria-hidden className="reward-live-dot h-1.5 w-1.5 shrink-0 rounded-full bg-primary" />
